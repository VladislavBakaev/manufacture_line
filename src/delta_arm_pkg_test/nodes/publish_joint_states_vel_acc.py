...
def move_of_trapeze_principle(msg):
    ...
    rate = rospy.Rate(2)
    while(not reachingOfThePoint(currentState.position,msg.position,0.1)):
        jointCmd.header.stamp = rospy.Time.now()
        jointPub.publish(jointCmd)
        rate.sleep()
        if(stop_move == True):
...
if __name__=='__main__':
    ...
    rospy.Subscriber('/delta_robot/joint_states',JointState,UpdateCurJointState)
